...
二段变身的机体如果武器太少，可能导致死机，死机时应该增加其武器和弹药槽数量，以保证残留的机体不会产生武器数量越界。
...
变形/合体/分离标志位为机体数据的第7位。所有机体形态必须连号。例如2段变形2号机的序号是2段变形1号机的序号+1。分离结果的机体序号是母体的序号+1。

另外，所有机体的武器库是共享的，所以所有机体的武器数量和弹药槽数量都需要一致。例如バトルクラフト的武器序号和数量和コンバトラーV一致。这也使得分离机体的改造段数可以继承到合体机体上。例子见后。
...
.. grid::  
    ...
    .. grid-item-card:: 未用
      ...
      | 28-2F：似乎等同于グレンダイザー
      | 30-34：似乎等同于ザンボット3
      | 38-3A：似乎等同于ダンクーガ的三段变形      
      | 3B-3C 可变形成为グレンダイザー的两个形态。变形时死机。
      | 3E 可变形成为ランドライガー的A和H两个形态。无法变回原机。
      | 3E 可变形成为マリンスペイザー或グレンダイザー（不可用）。变形后死机。
      | 3F 可变形成为マリンスペイザー或グレンダイザー（不可用）。无法变回原机。
      | 40-43 可分离（ダンクーガ）
      | 48-4C 可合体(コンバトラーV)
      | 50-57 可分离（コンバトラーV）
      | 58-5A 似乎等同于スーパーガンダム
...
这两个机体没有共通武器，所以需要放弃一些武器，让总体武器和残弹槽数量不超过原机体的最高值。ヒュッケバイン的武器和弹药槽数量0x07/0x04改成グルンガスト的0x10/0x06。

同时放弃ウイングガスト和ガストランダー两个形态以腾出更多武器序号的空间。因为武器库已经是共享的，所以ウイングガスト和ガストランダー的武器配置可以直接拿来改成单独的2段变形而不再需要调整武器。

没有弹药槽的EN和无消耗武器可以通过修改武器代码直接互换，但是有弹药槽的机体需要重新分配弹药槽以避免冲突。弹药槽的值存储在武器代码的高位，以弹药槽x4的方式储存。在此前提上，应尽可能地保留原有武器序号以避免死机。

.. grid::  

    .. grid-item-card:: ６０ミリバルカン
      :columns: auto
...
同一武器序号在不同形态中的武器编码可以不同。例如在ネオ・グランゾン/ヴァルシオーネR/ザムジード这个变形组合中，ザムジード的超振動拳和ネオ・グランゾン的グランワ－ムソ－ド以及ヴァルシオーネR的ディバインア－ム共享同一个武器序号。在变形时改造段数也可以共享。

变形之后残弹槽中的残弹不变。这在武器在不同形态弹药数量不同时，可能造成残弹过早耗尽的问题。