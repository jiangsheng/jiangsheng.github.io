...
这两个机体没有共通武器，所以需要放弃一些武器，让总体武器数量不超过原机体的最高值。同时放弃ウイングガスト和ガストランダー两个形态以腾出更多武器序号的空间。因为武器库已经是共享的，所以ウイングガスト和ガストランダー的武器配置可以直接拿来改成2段变形。

没有弹药槽的EN和无消耗武器可以直接替换，但是有弹药槽的机体需要重新分配弹药槽以避免冲突。在此前提上，尽可能地保留原有武器序号。
...
同一武器序号在不同形态中的武器编码可以不同。例如在ネオ・グランゾン/ヴァルシオーネR/ザムジード这个变形组合中，ザムジード的超振動拳和ネオ・グランゾン的グランワ－ムソ－ド以及ヴァルシオーネR的ディバインア－ム共享同一个武器序号。在变形时改造段数也可以移植。

变形之后残弹槽中的残弹不变。这在武器在不同形态编码不同时，可能造成残弹过早耗尽的问题。
