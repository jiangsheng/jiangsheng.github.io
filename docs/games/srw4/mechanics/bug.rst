...
* 第四次的深海地形，如果机体的移动类型没有海，一般格斗武器即使对海适应是A，也是潜不下去砍人的，只有1格的射击武器（比如泰坦光线）和远程武器可以用。但是一些武器不知为何算成了射击武器，可以对无法进入的地形使用。
  ...
  * \ :ref:`ドリルスパイザー <srw4_unit_drill_spider>`\ （钻地斯陪沙） 的ドリルアタック
...
* \ :doc:`../walkthrough/31_storm_of_kilimanjaro`\ 这一话，虽然地图上满是云看起来是空战，机体出击的时候，不能飞的不能出击，出击的也不能变形到不能飞的形态，然而地形适应是陆，对陆适应差的战斗会很吃力。\ :ref:`グレンダイザー (MS) <srw4_unit_grendizer_ms>`\ (潜水古连泰沙)和\ :ref:`グレンダイザー (DS) <srw4_unit_grendizer_ds>`\ (钻地古连泰沙)，虽然移动类型是水陆空和空陆地中，理论上不管地形算空还是算陆都可以进入的，合体之后就是移动不了或者直接死机，要装空飞芯片才能正常行动……
...
* 战场上合体出来的机器人，例如超级高达或者古连泰沙，在被击破时会爆掉一个飞机，只留下本体。但是如果是被搭载在母舰上，母舰被击坠的话，复活出来的会是合体状态的机器人。这个机器人可以分离，但是如果取消分离指令的话，就不能移动了，除非再次和飞机合体。
