.. meta::
   :description: 芯片是可以装备到你的机体的道具，用于改进机体性能，增加特殊功能，或者提供HP和EN回复的附件。 一个机体可以装备两个芯片。 每种芯片最大存储数量为9，超过的话，即使在地图上到达有芯片的地点，也不能获得芯片。 你可以在某个特定任务（通常是保护运送装备的运输机）完成之后获得一些芯片，但是大多数时候你可以通过移动到每关的特定
...
芯片是可以装备到你的机体的道具，用于改进机体性能，增加特殊功能，或者提供HP和EN回复的附件。 一个机体可以装备两个芯片。

每种芯片最大存储数量为9，超过的话，即使在地图上到达有芯片的地点，也不能获得芯片。

你可以在某个特定任务（通常是保护运送装备的运输机）完成之后获得一些芯片，但是大多数时候你可以通过移动到每关的特定地点来获得它们。地图上没有关于某个位置是否有芯片的标记，但是有些这类地点和周围不同，例如一个大森林中的空地。准确地探测芯片所在地点的唯一的方法是使用\ :ref:`精神“探索” <_srw4_mechanics_sprit_command>`\ ，每次使用获得一个地点而不是乱蒙一气。但是，除非你的主角有这个精神，否则在开始的10到11话之内，你的部队没有人有这个精神。我打算给出每一关的芯片位置，并在过关之后获得芯片时给出提示（但是还是有可能我错过了一些）
...
.. flat-table:: 芯片 （第四次超级机器人大战）
   ...
   * - 01
     ...
     - 移動形態を空陸に変更し
     - 将移动形态变为空陆
     - 格斗武器变成空A陆A。由于对空的适应力没有变化，而不能飞的机体多半完全没有对空适应，除了在\ :doc:`../walkthrough/31_storm_of_kilimanjaro`\ 这一话因为\ :ref:`bug <srw4_bugs>`十分有用之外，用处不大。
   * - 02
     - ブースター
     - 装備したユニットの移動力＋1
     - 装备的机体移动力+1
...
* :doc:`../walkthrough/07_battle_commander_daimos`\ 
...
* :doc:`../walkthrough/09_enter_zambot_3`
...
第四次芯片修改
-----------------------
.. _srw4_items_cheat_snes:

静态修改的话，芯片存储地址取决于是否有文件头，如果文件头存在则+200。未说明的数据为2字节。

在第四次的ROM中，芯片数据分两部分，第一部分为DF85C开始的数据，每个芯片4字节，共96字节。第二部分为DF8BC开始的数据，每个芯片4字节，共96字节。在金手指的时候，ROM地址加上C00000就是金手指地址。
