...
本作很多机体和机师的地形适应很差。例如GP-03デンドロビウム的移动类型是宇，最后几话如果不\ :ref:`修改 <srw4_cheat>`\ 的话不能出击。即使挂了芯片出击，地形适应也没有改善，所以攻防也非常差。所以要在选择出击队员甚至乘换的时候就考虑到地形适应。例如在宇宙地图派出适应为A的真实系，在派出适应为B的超级系的时候再三考虑。
...
如果你发现你位于敌方攻击范围内但是敌人却不攻击你，那可能你是位于敌人进入不了的地形。例如有些机体有对地或者海适用格斗武器，但是由于机体本身不能进入地面或者海，这些武器的这些适应实际上是摆好看的。一个例子是ガブスレイ(MA)的クローアーム，虽然地形适应是陆A海A，但是由于受机体本身移动类型限制，无法对陆上或者海中的敌人使用这个武器。这个武器不知为何在第四次中设定成了射击武器，所以尽管动画中仍旧是近身格斗，现在也可以对陆上或者海中的敌人使用。
