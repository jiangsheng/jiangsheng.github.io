.. meta::
   :description: ゲッターロボ/ゲッターロボG机体：括号内为PS版变动。地形补正(→)为用默认驾驶员的地形适应和机体的移动类型修正之后的数据。ゲッター1, ゲッター2, ゲッター3, ゲッタードラゴン, ゲッターライガー, ゲッターポセイドン, メカザウルス・サキ, メカザウルス・バド, メカザウルス・ザイ, メカザウルス・ゼンII, メカザウルス・ダイ, メカザウルス・シグ, 真・ゲッター1, 真・ゲッター2, 真・ゲッター3, テキサスマック, メカザウルス・ズー 
.. _srw4_units_getter_robo:

====================================
ゲッターロボ/ゲッターロボG登场机体
====================================

括号内为PS版变动。地形补正(→)为用默认驾驶员的地形适应和机体的移动类型修正之后的数据。偏差值表示排名位置，均值为50。

.. _srw4_units_getter_robo_commentBegin:


.. _srw4_units_getter_robo_commentEnd:

--------
我军机体
--------
.. _srw4_units_getter_robo_own_commentBegin:

盖塔小队的三个形态的改造段数中除了HP和EN之外都不共享，强化芯片也是独立的。

作为和泰坦3并列的最强超级机器人，不但在初期就能获得有助于培养的加速、热血、幸运、气合等精神，更是第一话就能加入队伍，实用性远超18话才入队的泰坦3。随着剧情推进，盖塔G和真盖塔会一次入手，无需吝惜资金。但是注意部分武器的改造段数不能继承到真盖塔。

和泰坦3比起来，装甲略薄，即使后期会获得HP回复能力，但是耐久度仍属一般，必须避免深入敌阵，而是在清理小兵之后收拾残局。由于EN消耗较大，建议配备EN回复芯片或者派遣补给机/机师出场。


.. _srw4_units_getter_robo_own_commentEnd:

^^^^^^^^^^
ゲッター1
^^^^^^^^^^

.. _srw4_unit_getter_1:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_18.png

        登场/加入:第1话

    .. grid-item::
        :columns: auto

        | HP 2800
        | 我军偏差值 48.28(48.27)
        | 整体偏差值 45.3(45.31)
        | EN 180
        | 我军偏差值 50.8(50.76)
        | 整体偏差值 47.2(47.18)
        | 装甲 280
        | 我军偏差值 44.95(44.83)
        | 整体偏差值 44.75(44.82)
        | 运动性 25
        | 我军偏差值 36.74(36.64)
        | 整体偏差值 39.41(39.37)
        | 限界 150
        | 我军偏差值 38.92(38.87)
        | 整体偏差值 39.34(39.34)

    .. grid-item::
        :columns: auto

        | 编码 18
        | Snes地址 B98EA
        | 武器首地址 B990A
        | PS地址 265D9
        | 类型 空陸
        | 移动力 8
        | 我军偏差值 47.89(47.89)
        | 整体偏差值 50.04(50.04)
        | 大小 L

        | 经验值 110
        | 我军偏差值 45.68(45.68)
        | 整体偏差值 46.44(46.44)
        | 价值 2400
        | 我军偏差值 47.65(47.65)
        | 整体偏差值 46.21(46.21)
        | 修理费 3200
        | 我军偏差值 47.34(47.34)
        | 整体偏差值 46.4(46.4)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆B
        | 海D
        | 宇A→B

        .. image:: ../pilots/images/srw4_pilot_09.png

        | 地形参照
        | :ref:`流竜馬 <srw4_pilots_ryoma_nagare>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ゲッターレザー🤛Ⓟ
        :columns: 3

    .. grid-item:: 670

    .. grid-item:: 1

    .. grid-item:: +30

    .. grid-item:: +20

    .. grid-item:: 空A陆A→B海A→C宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ｹﾞｯﾀｰﾄﾏﾎｰｸ🤛Ⓟ
        :columns: 3

    .. grid-item:: 800

    .. grid-item:: 1

    .. grid-item:: +25

    .. grid-item:: +10

    .. grid-item:: 空A陆A→B海A→C宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾄﾏﾎｰｸﾌﾞｰﾒﾗﾝ⚔Ⓟ
        :columns: 3

    .. grid-item:: 800

    .. grid-item:: 1~5

    .. grid-item:: +25

    .. grid-item:: +10

    .. grid-item:: 空A陆A海B宇B
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ゲッタービームⓅ
        :columns: 3

    .. grid-item:: 2300

    .. grid-item:: 1

    .. grid-item:: +5

    .. grid-item:: +20

    .. grid-item:: 空A陆A海🚫宇A
        :columns: 3

    .. grid-item:: EN 20

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_getter_1_commentBegin:

可以用到最终话而且改造也大多继承的强力机体，是重点培养和改造对象。注意不要改造ゲッターレザー，这个武器的改造不继承到真ゲッター。EN改造超过4级也将会是浪费资金。盖塔光线的消耗就威力来说非常小，如果要击落开局会逃窜的敌人，则应首先将其改造到5段以上再考虑其他机体的改造。

盖塔光线消耗EN虽然少但是也和后面增加的武器有点重复，但是不改造盖塔光线会损失一些资金（一些早期Boss会跑掉），除非有什么特别偏好，否则还是改造吧。

对于速通者来说，有射程5的移动后可对空武器也是个优势。缺点是弹数太少。

空中形态的盖塔1具有高移动力、高攻击，甚至有射程5-6的移动后可使用的远程武器，除了无法继承到后期的ゲッターレザー，其他武器都有改满的价值。相比之下其他两个形态因为地形和武器的限制，改造意义不大。

虽然是超级系，但是装甲脆得跟真实系有得一拼……或者你会换到2号机从地下接近敌人？

.. _srw4_unit_getter_1_commentEnd:

^^^^^^^^^^
ゲッター2
^^^^^^^^^^

.. _srw4_unit_getter_2:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_19.png

        登场/加入:第1话

    .. grid-item::
        :columns: auto

        | HP 2800
        | 我军偏差值 48.28(48.27)
        | 整体偏差值 45.3(45.31)
        | EN 180
        | 我军偏差值 50.8(50.76)
        | 整体偏差值 47.2(47.18)
        | 装甲 260
        | 我军偏差值 43.88(43.84)
        | 整体偏差值 43.9(44.01)
        | 运动性 30
        | 我军偏差值 42.37(42.28)
        | 整体偏差值 44.54(44.44)
        | 限界 160
        | 我军偏差值 42.01(41.97)
        | 整体偏差值 42.37(42.35)

    .. grid-item::
        :columns: auto

        | 编码 19
        | Snes地址 B9918
        | 武器首地址 B9938
        | PS地址 26607
        | 类型 陸地中
        | 移动力 10
        | 我军偏差值 58.72(58.72)
        | 整体偏差值 60.81(60.81)
        | 大小 L

        | 经验值 110
        | 我军偏差值 45.68(45.68)
        | 整体偏差值 46.44(46.44)
        | 价值 2400
        | 我军偏差值 47.65(47.65)
        | 整体偏差值 46.21(46.21)
        | 修理费 3200
        | 我军偏差值 47.34(47.34)
        | 整体偏差值 46.4(46.4)
    .. grid-item::
        :columns: auto

        | 空🚫→D
        | 陆A
        | 海C
        | 宇B
        | 分身

        .. image:: ../pilots/images/srw4_pilot_0A.png

        | 地形参照
        | :ref:`神隼人 <srw4_pilots_hayato_jin>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ゲッターアーム🤛Ⓟ
        :columns: 3

    .. grid-item:: 700

    .. grid-item:: 1

    .. grid-item:: +25

    .. grid-item:: +10

    .. grid-item:: 空A→🚫陆A海A→B宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ゲッタードリル🤛Ⓟ
        :columns: 3

    .. grid-item:: 820

    .. grid-item:: 1

    .. grid-item:: +5

    .. grid-item:: +20

    .. grid-item:: 空A→🚫陆A海A→B宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ドリルストームⓅ
        :columns: 3

    .. grid-item:: 1200

    .. grid-item:: 1

    .. grid-item:: 

    .. grid-item:: -10

    .. grid-item:: 空B陆A海B宇🚫
        :columns: 3

    .. grid-item:: EN 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ドリルパンチ⚔
        :columns: 3

    .. grid-item:: 1500

    .. grid-item:: 1~4

    .. grid-item:: -5

    .. grid-item:: +20

    .. grid-item:: 空C陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_getter_2_commentBegin:

盖塔三个形态中运动性和移动力最高的。但是也是最脆的，幸好有分身。攻击力也是最差的。仅用于赶路的形态，盖塔1对地乏力，2号机对地更强，幻影钻头仅需25EN，但是对空乏力。

到了中后期地面的敌人有挑战性的本来就不多，我方还有其他的陆战比空战出色的机体，这个形态也就前期有点用，所以不需要改造。

武器的ドリルパンチ的改造段数不能继承到真ゲッター。

.. _srw4_unit_getter_2_commentEnd:

^^^^^^^^^^
ゲッター3
^^^^^^^^^^

.. _srw4_unit_getter_3:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_1A.png

        登场/加入:第1话

    .. grid-item::
        :columns: auto

        | HP 2800
        | 我军偏差值 48.28(48.27)
        | 整体偏差值 45.3(45.31)
        | EN 180
        | 我军偏差值 50.8(50.76)
        | 整体偏差值 47.2(47.18)
        | 装甲 310
        | 我军偏差值 46.56(46.32)
        | 整体偏差值 46.04(46.03)
        | 运动性 23
        | 我军偏差值 34.49(34.38)
        | 整体偏差值 37.35(37.34)
        | 限界 130
        | 我军偏差值 32.73(32.68)
        | 整体偏差值 33.3(33.31)

    .. grid-item::
        :columns: auto

        | 编码 1A
        | Snes地址 B9946
        | 武器首地址 B9966
        | PS地址 26635
        | 类型 水陸
        | 移动力 6
        | 我军偏差值 37.07(37.07)
        | 整体偏差值 39.27(39.27)
        | 大小 L

        | 经验值 110
        | 我军偏差值 45.68(45.68)
        | 整体偏差值 46.44(46.44)
        | 价值 2400
        | 我军偏差值 47.65(47.65)
        | 整体偏差值 46.21(46.21)
        | 修理费 3200
        | 我军偏差值 47.34(47.34)
        | 整体偏差值 46.4(46.4)
    .. grid-item::
        :columns: auto

        | 空🚫→D
        | 陆A→B
        | 海A
        | 宇B

        .. image:: ../pilots/images/srw4_pilot_0B.png

        | 地形参照
        | :ref:`車弁慶 <srw4_pilots_benkei_kuruma>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: 格闘🤛Ⓟ
        :columns: 3

    .. grid-item:: 750

    .. grid-item:: 1

    .. grid-item:: +25

    .. grid-item:: -10

    .. grid-item:: 空A→🚫陆A→B海A宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ゲッターミサイル⚔
        :columns: 3

    .. grid-item:: 1800

    .. grid-item:: 1~6

    .. grid-item:: +5

    .. grid-item:: 

    .. grid-item:: 空C陆C海A宇C
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 大雪山おろし🤛Ⓟ
        :columns: 3

    .. grid-item:: 2000

    .. grid-item:: 1

    .. grid-item:: +20

    .. grid-item:: +30

    .. grid-item:: 空🚫陆A→B海A宇🚫
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 必要气力 120

    .. grid-item::
        :class: flex-break

.. _srw4_unit_getter_3_commentBegin:

在上一次大战中大放光彩的盖塔3号，本作中因为大雪山おろし的削弱而显得没有什么存在感。

无消耗的大雪山おろし可以很容易的消灭陆地和海中的敌人，但是对空能力太差，游戏后期敌人全员空飞的时候不装备ミノフスキークラフト则只有挨打。ミノフスキークラフト正常游戏只能获得一个，因为还有其他强力机体需要，基本没有人会浪费在本机身上。

本作因为海战太少的原因，也不需要改造，用的时候装个装甲就行。在某些特定地图，敌人的机动战士和重战机到我军的路中需要下水，这时候把ゲッター3单独放在水里可以让他们无法使用光线武器，一个个上来送死，是一个速通的小技巧。

.. _srw4_unit_getter_3_commentEnd:

^^^^^^^^^^^^^^^^
ゲッタードラゴン
^^^^^^^^^^^^^^^^

.. _srw4_unit_getter_dragon:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_1B.png

        登场/加入:第25话

    .. grid-item::
        :columns: auto

        | HP 3500
        | 我军偏差值 51.16(51.15)
        | 整体偏差值 45.89(45.89)
        | EN 210
        | 我军偏差值 59.61(59.57)
        | 整体偏差值 55.12(55.11)
        | 装甲 330
        | 我军偏差值 47.63(47.31)
        | 整体偏差值 46.89(46.83)
        | 运动性 30
        | 我军偏差值 42.37(42.28)
        | 整体偏差值 44.54(44.44)
        | 限界 180
        | 我军偏差值 48.2(48.16)
        | 整体偏差值 48.42(48.38)

    .. grid-item::
        :columns: auto

        | 编码 1B
        | Snes地址 B9972
        | 武器首地址 B9992
        | PS地址 26661
        | 类型 空陸
        | 移动力 9
        | 我军偏差值 53.31(53.31)
        | 整体偏差值 55.42(55.42)
        | 大小 L

        | 经验值 200
        | 我军偏差值 60.41(60.41)
        | 整体偏差值 60.24(60.24)
        | 价值 3800
        | 我军偏差值 50.28(50.28)
        | 整体偏差值 48.67(48.67)
        | 修理费 5500
        | 我军偏差值 49.91(49.91)
        | 整体偏差值 48.34(48.34)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆B
        | 海D
        | 宇A→B

        .. image:: ../pilots/images/srw4_pilot_09.png

        | 地形参照
        | :ref:`流竜馬 <srw4_pilots_ryoma_nagare>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: スピンカッター🤛Ⓟ
        :columns: 3

    .. grid-item:: 850

    .. grid-item:: 1

    .. grid-item:: +30

    .. grid-item:: +20

    .. grid-item:: 空A陆A→B海A→C宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ダブルトマホーク🤛Ⓟ
        :columns: 3

    .. grid-item:: 1200

    .. grid-item:: 1

    .. grid-item:: +25

    .. grid-item:: +10

    .. grid-item:: 空A陆A→B海A→C宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞﾌﾞﾙﾄﾏﾎｰｸﾌﾞｰﾒﾗﾝ⚔Ⓟ
        :columns: 3

    .. grid-item:: 1200

    .. grid-item:: 1~6

    .. grid-item:: +5

    .. grid-item:: +10

    .. grid-item:: 空A陆A海C宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ゲッタービームⓅ
        :columns: 3

    .. grid-item:: 2600

    .. grid-item:: 1

    .. grid-item:: +5

    .. grid-item:: +20

    .. grid-item:: 空A陆A海🚫宇A
        :columns: 3

    .. grid-item:: EN 20

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: シャインスパーク🤛Ⓟ
        :columns: 3

    .. grid-item:: 4000

    .. grid-item:: 1

    .. grid-item:: +5

    .. grid-item:: +10

    .. grid-item:: 空A陆A→B海🚫宇A→B
        :columns: 3

    .. grid-item:: EN 110

    .. grid-item:: 必要气力 120

    .. grid-item::
        :class: flex-break

.. _srw4_unit_getter_dragon_commentBegin:

盖塔龙有了强力的绝招，从此成长为Boss杀手。仍然很脆，所以变形成2号机潜行仍然有用。因为机师地形适应问题，シャインスパーク对地面敌人比较乏力。盖塔光线仍旧是对地面敌人的主战武器。闪光火花的改造会继承到真・ゲッター1，对空中敌人效果更佳，所以也需要改造。

对スピンカッター的修改不会被后续的真・ゲッター1继承。

.. _srw4_unit_getter_dragon_commentEnd:

^^^^^^^^^^^^^^^^
ゲッターライガー
^^^^^^^^^^^^^^^^

.. _srw4_unit_getter_liger:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_1C.png

        登场/加入:第25话

    .. grid-item::
        :columns: auto

        | HP 3500
        | 我军偏差值 51.16(51.15)
        | 整体偏差值 45.89(45.89)
        | EN 210
        | 我军偏差值 59.61(59.57)
        | 整体偏差值 55.12(55.11)
        | 装甲 290
        | 我军偏差值 45.49(45.33)
        | 整体偏差值 45.18(45.22)
        | 运动性 37
        | 我军偏差值 50.25(50.18)
        | 整体偏差值 51.73(51.55)
        | 限界 190
        | 我军偏差值 51.3(51.25)
        | 整体偏差值 51.44(51.4)

    .. grid-item::
        :columns: auto

        | 编码 1C
        | Snes地址 B99A3
        | 武器首地址 B99C3
        | PS地址 26692
        | 类型 陸地中
        | 移动力 12
        | 我军偏差值 69.54(69.54)
        | 整体偏差值 71.57(71.57)
        | 大小 L

        | 经验值 200
        | 我军偏差值 60.41(60.41)
        | 整体偏差值 60.24(60.24)
        | 价值 3800
        | 我军偏差值 50.28(50.28)
        | 整体偏差值 48.67(48.67)
        | 修理费 5500
        | 我军偏差值 49.91(49.91)
        | 整体偏差值 48.34(48.34)
    .. grid-item::
        :columns: auto

        | 空🚫→D
        | 陆A
        | 海B
        | 宇B
        | 分身

        .. image:: ../pilots/images/srw4_pilot_0A.png

        | 地形参照
        | :ref:`神隼人 <srw4_pilots_hayato_jin>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ドリルアーム🤛Ⓟ
        :columns: 3

    .. grid-item:: 1400

    .. grid-item:: 1

    .. grid-item:: +18

    .. grid-item:: +20

    .. grid-item:: 空A→🚫陆A海A→B宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: チェーンアタック🤛Ⓟ
        :columns: 3

    .. grid-item:: 1670

    .. grid-item:: 1

    .. grid-item:: +4

    .. grid-item:: +10

    .. grid-item:: 空A→🚫陆A海A→B宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ライガーミサイル⚔
        :columns: 3

    .. grid-item:: 1900

    .. grid-item:: 1~7

    .. grid-item:: 

    .. grid-item:: 

    .. grid-item:: 空C陆A海B宇B
        :columns: 3

    .. grid-item:: 残弹 3

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_getter_liger_commentBegin:

虽然盖特狮虎的移动力和GP-03有得一拼，但是无消耗的チェーンアタック威力不容小视。非常适合探宝之余顺路对付地面的杂鱼。另外ゲッター2的ドリルストーム的改造不被继承到本机，但是会继承到真ゲッター2的ドリルテンペスト上面去。本机新增的狮虎导弹的改造反而不被真ゲッター2继承。


.. _srw4_unit_getter_liger_commentEnd:

^^^^^^^^^^^^^^^^^^
ゲッターポセイドン
^^^^^^^^^^^^^^^^^^

.. _srw4_unit_getter_poseidon:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_1D.png

        登场/加入:第25话

    .. grid-item::
        :columns: auto

        | HP 3500
        | 我军偏差值 51.16(51.15)
        | 整体偏差值 45.89(45.89)
        | EN 210
        | 我军偏差值 59.61(59.57)
        | 整体偏差值 55.12(55.11)
        | 装甲 360
        | 我军偏差值 49.23(48.8)
        | 整体偏差值 48.17(48.04)
        | 运动性 26
        | 我军偏差值 37.87(37.77)
        | 整体偏差值 40.43(40.38)
        | 限界 170
        | 我军偏差值 45.11(45.06)
        | 整体偏差值 45.39(45.37)

    .. grid-item::
        :columns: auto

        | 编码 1D
        | Snes地址 B99CE
        | 武器首地址 B99EE
        | PS地址 266BD
        | 类型 水陸
        | 移动力 7
        | 我军偏差值 42.48(42.48)
        | 整体偏差值 44.66(44.66)
        | 大小 L

        | 经验值 200
        | 我军偏差值 60.41(60.41)
        | 整体偏差值 60.24(60.24)
        | 价值 3800
        | 我军偏差值 50.28(50.28)
        | 整体偏差值 48.67(48.67)
        | 修理费 5500
        | 我军偏差值 49.91(49.91)
        | 整体偏差值 48.34(48.34)
    .. grid-item::
        :columns: auto

        | 空🚫→D
        | 陆A→B
        | 海A
        | 宇B

        .. image:: ../pilots/images/srw4_pilot_0B.png

        | 地形参照
        | :ref:`車弁慶 <srw4_pilots_benkei_kuruma>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: 格闘🤛Ⓟ
        :columns: 3

    .. grid-item:: 820

    .. grid-item:: 1

    .. grid-item:: +25

    .. grid-item:: -10

    .. grid-item:: 空A→🚫陆A→B海A宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 大雪山おろし🤛Ⓟ
        :columns: 3

    .. grid-item:: 2000

    .. grid-item:: 1

    .. grid-item:: +20

    .. grid-item:: +30

    .. grid-item:: 空🚫陆A→B海A宇🚫
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 必要气力 120

    .. grid-item::
        :class: flex-break

    .. grid-item:: ｽﾄﾛﾝｸﾞﾐｻｲﾙ⚔
        :columns: 3

    .. grid-item:: 2100

    .. grid-item:: 1~6

    .. grid-item:: +3

    .. grid-item:: 

    .. grid-item:: 空C陆C海A宇C
        :columns: 3

    .. grid-item:: 残弹 3

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ｹﾞｯﾀｰｻｲｸﾛﾝⓅ
        :columns: 3

    .. grid-item:: 2200

    .. grid-item:: 1

    .. grid-item:: +12

    .. grid-item:: 

    .. grid-item:: 空🚫陆B海A宇🚫
        :columns: 3

    .. grid-item:: EN 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_getter_poseidon_commentBegin:

盖特波塞冬的大雪山おろし威力连导弹都不如……另外，对盖特旋风的改造并不会继承到真ゲッター3上。好在陆战还是能用的。

.. _srw4_unit_getter_poseidon_commentEnd:

^^^^^^^^^^^^^^
真・ゲッター1
^^^^^^^^^^^^^^

.. _srw4_unit_shin_getter_1:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_FC.png

        登场/加入:第35话

    .. grid-item::
        :columns: auto

        | HP 4100
        | 我军偏差值 53.62(53.61)
        | 整体偏差值 46.39(46.39)
        | EN 225
        | 我军偏差值 64.01(63.97)
        | 整体偏差值 59.08(59.07)
        | 装甲 400
        | 我军偏差值 51.37(50.78)
        | 整体偏差值 49.88(49.66)
        | 运动性 42
        | 我军偏差值 55.88(55.82)
        | 整体偏差值 56.86(56.63)
        | 限界 245
        | 我军偏差值 68.32(68.28)
        | 整体偏差值 68.07(67.98)

    .. grid-item::
        :columns: auto

        | 编码 FC
        | Snes地址 BC164
        | 武器首地址 BC184
        | PS地址 28E53
        | 类型 空陸
        | 移动力 12
        | 我军偏差值 69.54(69.54)
        | 整体偏差值 71.57(71.57)
        | 大小 L

        | 经验值 250
        | 我军偏差值 68.58(68.58)
        | 整体偏差值 67.91(67.91)
        | 价值 18000
        | 我军偏差值 76.95(76.95)
        | 整体偏差值 73.57(73.57)
        | 修理费 22000
        | 我军偏差值 68.34(68.34)
        | 整体偏差值 62.24(62.24)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆A→B
        | 海D
        | 宇A→B

        .. image:: ../pilots/images/srw4_pilot_09.png

        | 地形参照
        | :ref:`流竜馬 <srw4_pilots_ryoma_nagare>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ｹﾞｯﾀｰﾄﾏﾎｰｸ🤛Ⓟ
        :columns: 3

    .. grid-item:: 1500

    .. grid-item:: 1

    .. grid-item:: +5

    .. grid-item:: +30

    .. grid-item:: 空A陆A→B海A→C宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾄﾏﾎｰｸﾌﾞｰﾒﾗﾝ⚔Ⓟ
        :columns: 3

    .. grid-item:: 1800

    .. grid-item:: 1~6

    .. grid-item:: 

    .. grid-item:: +10

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ゲッタービームⓅ
        :columns: 3

    .. grid-item:: 2800

    .. grid-item:: 1

    .. grid-item:: 

    .. grid-item:: +20

    .. grid-item:: 空A陆A海🚫宇A
        :columns: 3

    .. grid-item:: EN 30

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ｽﾄﾅｰｻﾝｼｬｲﾝⓅ
        :columns: 3

    .. grid-item:: 5900

    .. grid-item:: 1

    .. grid-item:: -5

    .. grid-item:: +10

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: EN 120

    .. grid-item:: 必要气力 130

    .. grid-item::
        :class: flex-break

.. _srw4_unit_shin_getter_1_commentBegin:

真盖塔1的斯通纳阳光为全游戏攻击力前三，于核弹头这种太晚入手的武器不同，它入手很早，而且作为一格射击武器，没有闪光火花的地形使用问题。消耗EN120，不补给的话更是可以打两发。虽然机师没有魂，但是威力超越加了魂的太阳攻击和黑洞炮，当然还是不能和暗剑杀相比。

.. _srw4_unit_shin_getter_1_commentEnd:

^^^^^^^^^^^^^^
真・ゲッター2
^^^^^^^^^^^^^^

.. _srw4_unit_shin_getter_2:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_FD.png

        登场/加入:第35话

    .. grid-item::
        :columns: auto

        | HP 4100
        | 我军偏差值 53.62(53.61)
        | 整体偏差值 46.39(46.39)
        | EN 225
        | 我军偏差值 64.01(63.97)
        | 整体偏差值 59.08(59.07)
        | 装甲 350
        | 我军偏差值 48.7(48.3)
        | 整体偏差值 47.75(47.64)
        | 运动性 48
        | 我军偏差值 62.63(62.6)
        | 整体偏差值 63.02(62.72)
        | 限界 245
        | 我军偏差值 68.32(68.28)
        | 整体偏差值 68.07(67.98)

    .. grid-item::
        :columns: auto

        | 编码 FD
        | Snes地址 BC192
        | 武器首地址 BC1B2
        | PS地址 28E81
        | 类型 陸地中
        | 移动力 15
        | 我军偏差值 85.78(85.78)
        | 整体偏差值 87.72(87.72)
        | 大小 L

        | 经验值 250
        | 我军偏差值 68.58(68.58)
        | 整体偏差值 67.91(67.91)
        | 价值 18000
        | 我军偏差值 76.95(76.95)
        | 整体偏差值 73.57(73.57)
        | 修理费 22000
        | 我军偏差值 68.34(68.34)
        | 整体偏差值 62.24(62.24)
    .. grid-item::
        :columns: auto

        | 空🚫→D
        | 陆A
        | 海B
        | 宇A→B
        | 分身

        .. image:: ../pilots/images/srw4_pilot_0A.png

        | 地形参照
        | :ref:`神隼人 <srw4_pilots_hayato_jin>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ｹﾞｯﾀｰﾐｻｲﾙ⚔ (仅Snes)
        :columns: 3

    .. grid-item:: 1250

    .. grid-item:: 1~7

    .. grid-item:: -5

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 3

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ゲッタードリル🤛Ⓟ
        :columns: 3

    .. grid-item:: 1420

    .. grid-item:: 1

    .. grid-item:: +12

    .. grid-item:: +20

    .. grid-item:: 空A→🚫陆A海A→B宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾄﾞﾘﾙﾃﾝﾍﾟｽﾄ
        :columns: 3

    .. grid-item:: 2100

    .. grid-item:: 1~3

    .. grid-item:: 

    .. grid-item:: 

    .. grid-item:: 空🚫 (A)陆A海B宇🚫
        :columns: 3

    .. grid-item:: EN 5

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾐﾗｰｼﾞｭﾄﾞﾘﾙ🤛Ⓟ
        :columns: 3

    .. grid-item:: 3700

    .. grid-item:: 1

    .. grid-item:: +2

    .. grid-item:: +10

    .. grid-item:: 空A→🚫陆A海B→C宇A→B
        :columns: 3

    .. grid-item:: EN 25

    .. grid-item:: 必要气力 120

    .. grid-item::
        :class: flex-break

.. _srw4_unit_shin_getter_2_commentBegin:

第四次S取消了ゲッターミサイル这个弹药很少的远程武器，而消耗只有5EN的ドリルテンペスト的射程从1~3变成了1~7。从而变成了一个远程消耗机体。

.. _srw4_unit_shin_getter_2_commentEnd:

^^^^^^^^^^^^^^
真・ゲッター3
^^^^^^^^^^^^^^

.. _srw4_unit_shin_getter_3:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_FE.png

        登场/加入:第35话

    .. grid-item::
        :columns: auto

        | HP 4100
        | 我军偏差值 53.62(53.61)
        | 整体偏差值 46.39(46.39)
        | EN 225
        | 我军偏差值 64.01(63.97)
        | 整体偏差值 59.08(59.07)
        | 装甲 580
        | 我军偏差值 61.01(59.71)
        | 整体偏差值 57.58(56.91)
        | 运动性 37
        | 我军偏差值 50.25(50.18)
        | 整体偏差值 51.73(51.55)
        | 限界 245
        | 我军偏差值 68.32(68.28)
        | 整体偏差值 68.07(67.98)

    .. grid-item::
        :columns: auto

        | 编码 FE
        | Snes地址 BC1C0
        | 武器首地址 BC1E0
        | PS地址 28EAC
        | 类型 水陸
        | 移动力 8
        | 我军偏差值 47.89(47.89)
        | 整体偏差值 50.04(50.04)
        | 大小 L

        | 经验值 250
        | 我军偏差值 68.58(68.58)
        | 整体偏差值 67.91(67.91)
        | 价值 18000
        | 我军偏差值 76.95(76.95)
        | 整体偏差值 73.57(73.57)
        | 修理费 22000
        | 我军偏差值 68.34(68.34)
        | 整体偏差值 62.24(62.24)
    .. grid-item::
        :columns: auto

        | 空🚫→D
        | 陆A→B
        | 海A
        | 宇B

        .. image:: ../pilots/images/srw4_pilot_0B.png

        | 地形参照
        | :ref:`車弁慶 <srw4_pilots_benkei_kuruma>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ｹﾞｯﾀｰﾐｻｲﾙ⚔
        :columns: 3

    .. grid-item:: 1500

    .. grid-item:: 1~6

    .. grid-item:: -18

    .. grid-item:: -10

    .. grid-item:: 空B陆B海A宇B
        :columns: 3

    .. grid-item:: 残弹 3

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ハンマーパンチ🤛Ⓟ
        :columns: 3

    .. grid-item:: 1750

    .. grid-item:: 1

    .. grid-item:: +4

    .. grid-item:: +10

    .. grid-item:: 空A→🚫陆A→B海A宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 大雪山おろし二段返し🤛Ⓟ
        :columns: 3

    .. grid-item:: 4200(4600)

    .. grid-item:: 1

    .. grid-item:: +18

    .. grid-item:: +30

    .. grid-item:: 空🚫陆A→B海A宇🚫
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 必要气力 130

    .. grid-item::
        :class: flex-break

.. _srw4_unit_shin_getter_3_commentBegin:


.. _srw4_unit_shin_getter_3_commentEnd:

^^^^^^^^^^^^^^
テキサスマック
^^^^^^^^^^^^^^

.. _srw4_unit_texas_mack:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_10A.png

        登场/加入:第4话

    .. grid-item::
        :columns: auto

        | HP 2800
        | 我军偏差值 48.28(48.27)
        | 整体偏差值 45.3(45.31)
        | EN 180
        | 我军偏差值 50.8(50.76)
        | 整体偏差值 47.2(47.18)
        | 装甲 300
        | 我军偏差值 46.02(45.83)
        | 整体偏差值 45.61(45.62)
        | 运动性 26
        | 我军偏差值 37.87(37.77)
        | 整体偏差值 40.43(40.38)
        | 限界 150
        | 我军偏差值 38.92(38.87)
        | 整体偏差值 39.34(39.34)

    .. grid-item::
        :columns: auto

        | 编码 10A
        | Snes地址 BC37A
        | 武器首地址 BC39A
        | PS地址 29066
        | 类型 水陸空
        | 移动力 9
        | 我军偏差值 53.31(53.31)
        | 整体偏差值 55.42(55.42)
        | 大小 M

        | 经验值 90
        | 我军偏差值 42.41(42.41)
        | 整体偏差值 43.37(43.37)
        | 价值 2000
        | 我军偏差值 46.9(46.9)
        | 整体偏差值 45.51(45.51)
        | 修理费 3000
        | 我军偏差值 47.11(47.11)
        | 整体偏差值 46.23(46.23)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆A
        | 海A
        | 宇B→C

        .. image:: ../pilots/images/srw4_pilot_07.png

        | 地形参照
        | :ref:`ジャック・キング <srw4_pilots_jack_king>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: テキサスソード🤛Ⓟ
        :columns: 3

    .. grid-item:: 940

    .. grid-item:: 1

    .. grid-item:: +15

    .. grid-item:: +10

    .. grid-item:: 空A陆A海A宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: マックライアット
        :columns: 3

    .. grid-item:: 1000

    .. grid-item:: 1~3

    .. grid-item:: +25

    .. grid-item:: -10

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: マックリボルバーⓅ
        :columns: 3

    .. grid-item:: 1050

    .. grid-item:: 1

    .. grid-item:: 

    .. grid-item:: +10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 6

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: マックライフル
        :columns: 3

    .. grid-item:: 1200

    .. grid-item:: 1~7

    .. grid-item:: 

    .. grid-item:: +10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_texas_mack_commentBegin:

只在速路线临时出战一话，并不会加入。真实系的攻击力，超级系的运动性……除了那些死要钱的（盖塔队还没幸运），还是当专职探宝队吧。

.. _srw4_unit_texas_mack_commentEnd:

--------
敌军机体
--------
.. _srw4_units_getter_robo_enemy_commentBegin:


.. _srw4_units_getter_robo_enemy_commentEnd:

^^^^^^^^^^^^^^^^^^
メカザウルス・バド
^^^^^^^^^^^^^^^^^^

.. _srw4_unit_mechasaurus_bado:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_90.png

        登场/加入:第1话

    .. grid-item::
        :columns: auto

        | HP 1800
        | 我军偏差值 44.17(44.17)
        | 整体偏差值 44.46(44.48)
        | EN 200
        | 我军偏差值 56.67(56.63)
        | 整体偏差值 52.48(52.47)
        | 装甲 120
        | 我军偏差值 36.39(36.9)
        | 整体偏差值 37.91(38.37)
        | 运动性 24
        | 我军偏差值 35.61(35.51)
        | 整体偏差值 38.38(38.35)
        | 限界 140
        | 我军偏差值 35.82(35.77)
        | 整体偏差值 36.32(36.32)

    .. grid-item::
        :columns: auto

        | 编码 90
        | Snes地址 BAED1
        | 武器首地址 BAEF1
        | PS地址 27BC0
        | 类型 空
        | 移动力 6
        | 我军偏差值 37.07(37.07)
        | 整体偏差值 39.27(39.27)
        | 大小 L

        | 经验值 40
        | 我军偏差值 34.24(34.24)
        | 整体偏差值 35.71(35.71)
        | 价值 500
        | 我军偏差值 44.08(44.08)
        | 整体偏差值 42.88(42.88)
        | 修理费 100
        | 我军偏差值 43.87(43.87)
        | 整体偏差值 43.79(43.79)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆🚫
        | 海🚫
        | 宇🚫


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: マグマ弾⚔Ⓟ
        :columns: 3

    .. grid-item:: 620

    .. grid-item:: 1

    .. grid-item:: +10

    .. grid-item:: -10

    .. grid-item:: 空A陆A海C宇🚫
        :columns: 3

    .. grid-item:: 残弹 6

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ミサイル⚔
        :columns: 3

    .. grid-item:: 880

    .. grid-item:: 1~5

    .. grid-item:: 

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mechasaurus_bado_commentBegin:

和ドローメ的区别是一下打不死。

.. _srw4_unit_mechasaurus_bado_commentEnd:

^^^^^^^^^^^^^^^^^^
メカザウルス・サキ
^^^^^^^^^^^^^^^^^^

.. _srw4_unit_mechasaurus_saki:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_8F.png

        登场/加入:第2话

    .. grid-item::
        :columns: auto

        | HP 3000
        | 我军偏差值 49.1(49.09)
        | 整体偏差值 45.47(45.48)
        | EN 200
        | 我军偏差值 56.67(56.63)
        | 整体偏差值 52.48(52.47)
        | 装甲 180
        | 我军偏差值 39.6(39.88)
        | 整体偏差值 40.48(40.79)
        | 运动性 20
        | 我军偏差值 31.11(30.99)
        | 整体偏差值 34.27(34.29)
        | 限界 130
        | 我军偏差值 32.73(32.68)
        | 整体偏差值 33.3(33.31)

    .. grid-item::
        :columns: auto

        | 编码 8F
        | Snes地址 BAEA9
        | 武器首地址 BAEC9
        | PS地址 27B98
        | 类型 陸
        | 移动力 6
        | 我军偏差值 37.07(37.07)
        | 整体偏差值 39.27(39.27)
        | 大小 L

        | 经验值 50
        | 我军偏差值 35.87(35.87)
        | 整体偏差值 37.24(37.24)
        | 价值 800
        | 我军偏差值 44.64(44.64)
        | 整体偏差值 43.41(43.41)
        | 修理费 100
        | 我军偏差值 43.87(43.87)
        | 整体偏差值 43.79(43.79)
    .. grid-item::
        :columns: auto

        | 空🚫
        | 陆A
        | 海B
        | 宇🚫


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ミニミサイル⚔
        :columns: 3

    .. grid-item:: 680

    .. grid-item:: 1~5

    .. grid-item:: -10

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 14

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: クロー🤛Ⓟ
        :columns: 3

    .. grid-item:: 790

    .. grid-item:: 1

    .. grid-item:: +5

    .. grid-item:: 

    .. grid-item:: 空A→🚫陆A海A→B宇A→🚫
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mechasaurus_saki_commentBegin:

武器最高只有790还不能对空的肉靶子。

.. _srw4_unit_mechasaurus_saki_commentEnd:

^^^^^^^^^^^^^^^^^^
メカザウルス・ザイ
^^^^^^^^^^^^^^^^^^

.. _srw4_unit_mechasaurus_zai:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_91.png

        登场/加入:第3话

    .. grid-item::
        :columns: auto

        | HP 2500
        | 我军偏差值 47.05(47.04)
        | 整体偏差值 45.05(45.06)
        | EN 200
        | 我军偏差值 56.67(56.63)
        | 整体偏差值 52.48(52.47)
        | 装甲 350
        | 我军偏差值 48.7(48.3)
        | 整体偏差值 47.75(47.64)
        | 运动性 24
        | 我军偏差值 35.61(35.51)
        | 整体偏差值 38.38(38.35)
        | 限界 140
        | 我军偏差值 35.82(35.77)
        | 整体偏差值 36.32(36.32)

    .. grid-item::
        :columns: auto

        | 编码 91
        | Snes地址 BAEF9
        | 武器首地址 BAF19
        | PS地址 27BE8
        | 类型 陸
        | 移动力 5
        | 我军偏差值 31.65(31.65)
        | 整体偏差值 33.89(33.89)
        | 大小 L

        | 经验值 50
        | 我军偏差值 35.87(35.87)
        | 整体偏差值 37.24(37.24)
        | 价值 900
        | 我军偏差值 44.83(44.83)
        | 整体偏差值 43.58(43.58)
        | 修理费 100
        | 我军偏差值 43.87(43.87)
        | 整体偏差值 43.79(43.79)
    .. grid-item::
        :columns: auto

        | 空🚫
        | 陆A
        | 海B
        | 宇🚫


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ミニミサイル⚔
        :columns: 3

    .. grid-item:: 680

    .. grid-item:: 1~5

    .. grid-item:: -10

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 14

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 体当たり🤛Ⓟ
        :columns: 3

    .. grid-item:: 1650

    .. grid-item:: 1

    .. grid-item:: +5

    .. grid-item:: +10

    .. grid-item:: 空A→🚫陆A海A→B宇B→🚫
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mechasaurus_zai_commentBegin:

虽然很脆但是有不错的近身武器，如果不是移动力太差的话或许有点威胁。

.. _srw4_unit_mechasaurus_zai_commentEnd:

^^^^^^^^^^^^^^^^^^^^^^
メカザウルス・ゼンII
^^^^^^^^^^^^^^^^^^^^^^

.. _srw4_unit_mechasaurus_zen_ii:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_92.png

        登场/加入:第3话

    .. grid-item::
        :columns: auto

        | HP 3300
        | 我军偏差值 50.34(50.33)
        | 整体偏差值 45.72(45.73)
        | EN 200
        | 我军偏差值 56.67(56.63)
        | 整体偏差值 52.48(52.47)
        | 装甲 300
        | 我军偏差值 46.02(45.83)
        | 整体偏差值 45.61(45.62)
        | 运动性 28
        | 我军偏差值 40.12(40.02)
        | 整体偏差值 42.49(42.41)
        | 限界 150
        | 我军偏差值 38.92(38.87)
        | 整体偏差值 39.34(39.34)

    .. grid-item::
        :columns: auto

        | 编码 92
        | Snes地址 BAF21
        | 武器首地址 BAF41
        | PS地址 27C10
        | 类型 陸
        | 移动力 6
        | 我军偏差值 37.07(37.07)
        | 整体偏差值 39.27(39.27)
        | 大小 L

        | 经验值 60
        | 我军偏差值 37.51(37.51)
        | 整体偏差值 38.77(38.77)
        | 价值 1200
        | 我军偏差值 45.4(45.4)
        | 整体偏差值 44.11(44.11)
        | 修理费 100
        | 我军偏差值 43.87(43.87)
        | 整体偏差值 43.79(43.79)
    .. grid-item::
        :columns: auto

        | 空🚫
        | 陆A
        | 海B
        | 宇🚫


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ロケット弾⚔
        :columns: 3

    .. grid-item:: 700

    .. grid-item:: 1~5

    .. grid-item:: -5

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ハンドソルド🤛Ⓟ
        :columns: 3

    .. grid-item:: 980

    .. grid-item:: 1

    .. grid-item:: +8

    .. grid-item:: +10

    .. grid-item:: 空A→🚫陆A海A→B宇A→🚫
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mechasaurus_zen_ii_commentBegin:

跟機械獣ジェノバM9差不多，但是射程和攻击力更差。

.. _srw4_unit_mechasaurus_zen_ii_commentEnd:

^^^^^^^^^^^^^^^^^^
メカザウルス・ズー
^^^^^^^^^^^^^^^^^^

.. _srw4_unit_mechasaurus_zuu:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_11E.png

        登场/加入:第4话

    .. grid-item::
        :columns: auto

        | HP 3000
        | 我军偏差值 49.1(49.09)
        | 整体偏差值 45.47(45.48)
        | EN 200
        | 我军偏差值 56.67(56.63)
        | 整体偏差值 52.48(52.47)
        | 装甲 300
        | 我军偏差值 46.02(45.83)
        | 整体偏差值 45.61(45.62)
        | 运动性 24
        | 我军偏差值 35.61(35.51)
        | 整体偏差值 38.38(38.35)
        | 限界 140
        | 我军偏差值 35.82(35.77)
        | 整体偏差值 36.32(36.32)

    .. grid-item::
        :columns: auto

        | 编码 11E
        | Snes地址 BC6DF
        | 武器首地址 BC6FF
        | PS地址 293CB
        | 类型 水陸
        | 移动力 5
        | 我军偏差值 31.65(31.65)
        | 整体偏差值 33.89(33.89)
        | 大小 M

        | 经验值 50
        | 我军偏差值 35.87(35.87)
        | 整体偏差值 37.24(37.24)
        | 价值 800
        | 我军偏差值 44.64(44.64)
        | 整体偏差值 43.41(43.41)
        | 修理费 100
        | 我军偏差值 43.87(43.87)
        | 整体偏差值 43.79(43.79)
    .. grid-item::
        :columns: auto

        | 空🚫
        | 陆B
        | 海A
        | 宇🚫


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: しっぽ🤛Ⓟ
        :columns: 3

    .. grid-item:: 750

    .. grid-item:: 1

    .. grid-item:: +15

    .. grid-item:: +10

    .. grid-item:: 空A→🚫陆A→B海A宇A→🚫
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ミサイル⚔
        :columns: 3

    .. grid-item:: 880

    .. grid-item:: 1~5

    .. grid-item:: 

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 火炎🤛Ⓟ
        :columns: 3

    .. grid-item:: 1000

    .. grid-item:: 1

    .. grid-item:: +10

    .. grid-item:: -10

    .. grid-item:: 空A→🚫陆A→B海🚫宇🚫
        :columns: 3

    .. grid-item:: EN 5

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mechasaurus_zuu_commentBegin:

虽然是海战专用机体，但是大招居然不能对海……

.. _srw4_unit_mechasaurus_zuu_commentEnd:

^^^^^^^^^^^^^^^^^^
メカザウルス・ダイ
^^^^^^^^^^^^^^^^^^

.. _srw4_unit_mechasaurus_dai:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_93.png

        登场/加入:第15话

    .. grid-item::
        :columns: auto

        | HP 20000
        | 我军偏差值 118.93(118.9)
        | 整体偏差值 59.76(59.6)
        | EN 255
        | 我军偏差值 72.83(72.77)
        | 整体偏差值 67(66.99)
        | 装甲 490
        | 我军偏差值 56.19(55.25)
        | 整体偏差值 53.73(53.28)
        | 运动性 20
        | 我军偏差值 31.11(30.99)
        | 整体偏差值 34.27(34.29)
        | 限界 150
        | 我军偏差值 38.92(38.87)
        | 整体偏差值 39.34(39.34)

    .. grid-item::
        :columns: auto

        | 编码 93
        | Snes地址 BAF49
        | 武器首地址 BAF69
        | PS地址 27C38
        | 类型 水陸
        | 移动力 5
        | 我军偏差值 31.65(31.65)
        | 整体偏差值 33.89(33.89)
        | 大小 LL

        | 经验值 110
        | 我军偏差值 45.68(45.68)
        | 整体偏差值 46.44(46.44)
        | 价值 4000
        | 我军偏差值 50.66(50.66)
        | 整体偏差值 49.02(49.02)
        | 修理费 2000
        | 我军偏差值 46(46)
        | 整体偏差值 45.39(45.39)
    .. grid-item::
        :columns: auto

        | 空🚫
        | 陆A
        | 海A
        | 宇🚫


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ミニミサイル⚔
        :columns: 3

    .. grid-item:: 680

    .. grid-item:: 1~5

    .. grid-item:: -10

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 14

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 大砲⚔
        :columns: 3

    .. grid-item:: 1500

    .. grid-item:: 1~7

    .. grid-item:: -15

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 20

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mechasaurus_dai_commentBegin:


.. _srw4_unit_mechasaurus_dai_commentEnd:

^^^^^^^^^^^^^^^^^^
メカザウルス・シグ
^^^^^^^^^^^^^^^^^^

.. _srw4_unit_mechasaurus_shigu:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_94.png

        登场/加入:第15话

    .. grid-item::
        :columns: auto

        | HP 7200
        | 我军偏差值 66.35(66.34)
        | 整体偏差值 49(48.96)
        | EN 255
        | 我军偏差值 72.83(72.77)
        | 整体偏差值 67(66.99)
        | 装甲 470
        | 我军偏差值 55.12(54.25)
        | 整体偏差值 52.88(52.48)
        | 运动性 32
        | 我军偏差值 44.62(44.54)
        | 整体偏差值 46.59(46.47)
        | 限界 160
        | 我军偏差值 42.01(41.97)
        | 整体偏差值 42.37(42.35)

    .. grid-item::
        :columns: auto

        | 编码 94
        | Snes地址 BAF71
        | 武器首地址 BAF91
        | PS地址 27C60
        | 类型 水陸空
        | 移动力 8
        | 我军偏差值 47.89(47.89)
        | 整体偏差值 50.04(50.04)
        | 大小 L

        | 经验值 100
        | 我军偏差值 44.05(44.05)
        | 整体偏差值 44.91(44.91)
        | 价值 2800
        | 我军偏差值 48.4(48.4)
        | 整体偏差值 46.91(46.91)
        | 修理费 1000
        | 我军偏差值 44.88(44.88)
        | 整体偏差值 44.55(44.55)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆A
        | 海A
        | 宇🚫


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: クロー🤛Ⓟ
        :columns: 3

    .. grid-item:: 1200

    .. grid-item:: 1

    .. grid-item:: +5

    .. grid-item:: 

    .. grid-item:: 空A陆A海A宇A→🚫
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: レーザー
        :columns: 3

    .. grid-item:: 1250

    .. grid-item:: 1~6

    .. grid-item:: -5

    .. grid-item:: 

    .. grid-item:: 空A陆A海🚫宇A
        :columns: 3

    .. grid-item:: 残弹 5

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 破壊光線
        :columns: 3

    .. grid-item:: 2100

    .. grid-item:: 1~6

    .. grid-item:: -20

    .. grid-item:: 

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 18

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mechasaurus_shigu_commentBegin:


.. _srw4_unit_mechasaurus_shigu_commentEnd:

