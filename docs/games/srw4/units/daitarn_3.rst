.. meta::
   :description: ダイターン3机体：括号内为PS版变动。地形补正(→)为用默认驾驶员的地形适应和机体的移动类型修正之后的数据。ダイターン3, ダイファイター, ダイタンク 
.. _srw4_units_daitarn_3:

====================
ダイターン3登场机体
====================

括号内为PS版变动。地形补正(→)为用默认驾驶员的地形适应和机体的移动类型修正之后的数据。偏差值表示排名位置，均值为50。

.. _srw4_units_daitarn_3_commentBegin:


.. _srw4_units_daitarn_3_commentEnd:

--------
我军机体
--------
.. _srw4_units_daitarn_3_own_commentBegin:



.. _srw4_units_daitarn_3_own_commentEnd:

^^^^^^^^^^^^
ダイターン3
^^^^^^^^^^^^

.. _srw4_unit_daitarn_3:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_37.png

        登场/加入:第18话

    .. grid-item::
        :columns: auto

        | HP 4700
        | 我军偏差值 56.09(56.07)
        | 整体偏差值 46.9(46.89)
        | EN 255
        | 我军偏差值 72.83(72.77)
        | 整体偏差值 67(66.99)
        | 装甲 600
        | 我军偏差值 62.08(60.7)
        | 整体偏差值 58.43(57.72)
        | 运动性 38
        | 我军偏差值 51.37(51.31)
        | 整体偏差值 52.75(52.57)
        | 限界 185
        | 我军偏差值 49.75(49.7)
        | 整体偏差值 49.93(49.89)

    .. grid-item::
        :columns: auto

        | 编码 37
        | Snes地址 B9F2A
        | 武器首地址 B9F4A
        | PS地址 26C19
        | 类型 空陸
        | 移动力 8
        | 我军偏差值 47.89
        | 整体偏差值 50.04
        | 大小 LL

        | 经验值 250
        | 我军偏差值 68.58
        | 整体偏差值 67.91
        | 价值 5500
        | 我军偏差值 53.47
        | 整体偏差值 51.65
        | 修理费 8000
        | 我军偏差值 52.7
        | 整体偏差值 50.45
    .. grid-item::
        :columns: auto

        | 空A
        | 陆A
        | 海C→B
        | 宇A
        | 剣装備

        .. image:: ../pilots/images/srw4_pilot_6A.png

        | 地形参照
        | :ref:`破嵐万丈 <srw4_pilots_banjo_haran>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞｲﾀｰﾝｳｪｯﾌﾞ⚔Ⓟ
        :columns: 3

    .. grid-item:: 1200

    .. grid-item:: 1~3

    .. grid-item:: 

    .. grid-item:: +20

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞｲﾀｰﾝﾐｻｲﾙ⚔
        :columns: 3

    .. grid-item:: 1280

    .. grid-item:: 1~6

    .. grid-item:: -8

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ビッグウェッブ⚔
        :columns: 3

    .. grid-item:: 1500

    .. grid-item:: 1~6

    .. grid-item:: -5

    .. grid-item:: +20

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 1

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞｲﾀｰﾝｻﾞﾝﾊﾞｰ🤛Ⓟ
        :columns: 3

    .. grid-item:: 1670

    .. grid-item:: 1

    .. grid-item:: +15

    .. grid-item:: +20

    .. grid-item:: 空A陆A海A→B宇A
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞｲﾀｰﾝﾊﾝﾏｰ🤛Ⓟ
        :columns: 3

    .. grid-item:: 1940

    .. grid-item:: 1

    .. grid-item:: -5

    .. grid-item:: +10

    .. grid-item:: 空A陆A海A→B宇A
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: サンレーザーⓅ
        :columns: 3

    .. grid-item:: 2000

    .. grid-item:: 1

    .. grid-item:: -5

    .. grid-item:: +10

    .. grid-item:: 空A陆A海C宇A
        :columns: 3

    .. grid-item:: EN 25

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞｲﾀｰﾝｷｬﾉﾝ⚔
        :columns: 3

    .. grid-item:: 2100

    .. grid-item:: 1~7

    .. grid-item:: -5

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 8

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: サンアタックⓅ
        :columns: 3

    .. grid-item:: 3900

    .. grid-item:: 1

    .. grid-item:: +15

    .. grid-item:: +30

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: EN 70

    .. grid-item:: 必要气力 130

    .. grid-item::
        :class: flex-break

.. _srw4_unit_daitarn_3_commentBegin:

泰坦3和盖塔为我军最强机器人。由于机师SP充沛，更兼有魂，单座也不是什么问题。无消耗和低消耗的武器众多，有泰坦锤、泰坦斩和线激光可以选择作为主力改造对象。因机体装甲不错，也可以改造泰坦加农射程作为反击武器。太阳攻击为Boss杀手，应尽早改造。

.. _srw4_unit_daitarn_3_commentEnd:

^^^^^^^^^^^^^^
ダイファイター
^^^^^^^^^^^^^^

.. _srw4_unit_dai_fighter:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_38.png

        登场/加入:第18话

    .. grid-item::
        :columns: auto

        | HP 4700
        | 我军偏差值 56.09(56.07)
        | 整体偏差值 46.9(46.89)
        | EN 255
        | 我军偏差值 72.83(72.77)
        | 整体偏差值 67(66.99)
        | 装甲 570
        | 我军偏差值 60.47(59.21)
        | 整体偏差值 57.15(56.51)
        | 运动性 34
        | 我军偏差值 46.87(46.79)
        | 整体偏差值 48.65(48.5)
        | 限界 180
        | 我军偏差值 48.2(48.16)
        | 整体偏差值 48.42(48.38)

    .. grid-item::
        :columns: auto

        | 编码 38
        | Snes地址 B9F64
        | 武器首地址 B9F84
        | PS地址 26C53
        | 类型 空
        | 移动力 10
        | 我军偏差值 58.72
        | 整体偏差值 60.81
        | 大小 LL

        | 经验值 250
        | 我军偏差值 68.58
        | 整体偏差值 67.91
        | 价值 5500
        | 我军偏差值 53.47
        | 整体偏差值 51.65
        | 修理费 8000
        | 我军偏差值 52.7
        | 整体偏差值 50.45
    .. grid-item::
        :columns: auto

        | 空A
        | 陆🚫→C
        | 海🚫→C
        | 宇A

        .. image:: ../pilots/images/srw4_pilot_6A.png

        | 地形参照
        | :ref:`破嵐万丈 <srw4_pilots_banjo_haran>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞｲﾀｰﾝﾐｻｲﾙ⚔
        :columns: 3

    .. grid-item:: 1280

    .. grid-item:: 1~6

    .. grid-item:: -8

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ロケット砲⚔
        :columns: 3

    .. grid-item:: 1900

    .. grid-item:: 1~7

    .. grid-item:: -5

    .. grid-item:: -10

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 8

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 爆弾⚔Ⓟ
        :columns: 3

    .. grid-item:: 1940

    .. grid-item:: 1

    .. grid-item:: +20

    .. grid-item:: -10

    .. grid-item:: 空🚫陆A海A宇🚫
        :columns: 3

    .. grid-item:: 残弹 3

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: サンレーザーⓅ
        :columns: 3

    .. grid-item:: 2000

    .. grid-item:: 1

    .. grid-item:: -5

    .. grid-item:: +10

    .. grid-item:: 空A陆A海C宇A
        :columns: 3

    .. grid-item:: EN 25

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_dai_fighter_commentBegin:


.. _srw4_unit_dai_fighter_commentEnd:

^^^^^^^^^^
ダイタンク
^^^^^^^^^^

.. _srw4_unit_dai_tank:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_39.png

        登场/加入:第18话

    .. grid-item::
        :columns: auto

        | HP 4700
        | 我军偏差值 56.09(56.07)
        | 整体偏差值 46.9(46.89)
        | EN 255
        | 我军偏差值 72.83(72.77)
        | 整体偏差值 67(66.99)
        | 装甲 750
        | 我军偏差值 70.11(68.14)
        | 整体偏差值 64.85(63.77)
        | 运动性 28
        | 我军偏差值 40.12(40.02)
        | 整体偏差值 42.49(42.41)
        | 限界 160
        | 我军偏差值 42.01(41.97)
        | 整体偏差值 42.37(42.35)

    .. grid-item::
        :columns: auto

        | 编码 39
        | Snes地址 B9F92
        | 武器首地址 B9FB2
        | PS地址 26C81
        | 类型 水陸
        | 移动力 6
        | 我军偏差值 37.07
        | 整体偏差值 39.27
        | 大小 L

        | 经验值 250
        | 我军偏差值 68.58
        | 整体偏差值 67.91
        | 价值 5500
        | 我军偏差值 53.47
        | 整体偏差值 51.65
        | 修理费 8000
        | 我军偏差值 52.7
        | 整体偏差值 50.45
    .. grid-item::
        :columns: auto

        | 空🚫→C
        | 陆A
        | 海A
        | 宇C→B

        .. image:: ../pilots/images/srw4_pilot_6A.png

        | 地形参照
        | :ref:`破嵐万丈 <srw4_pilots_banjo_haran>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞｲﾀｰﾝﾐｻｲﾙ⚔
        :columns: 3

    .. grid-item:: 1280

    .. grid-item:: 1~6

    .. grid-item:: -8

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞｲﾀｰﾝｷｬﾉﾝ⚔
        :columns: 3

    .. grid-item:: 2100

    .. grid-item:: 1~7

    .. grid-item:: -5

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 8

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_dai_tank_commentBegin:


.. _srw4_unit_dai_tank_commentEnd:

