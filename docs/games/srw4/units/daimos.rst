.. meta::
   :description: ダイモス机体：括号内为PS版变动。地形补正(→)为用默认驾驶员的地形适应和机体的移动类型修正之后的数据。ダイモス, ガルバーFXⅡ, メカ戦士ゾンネカイザー, メカ戦士ギメリア, メカ戦士ゴッドアーモン, ガルンロール 
.. _srw4_units_daimos:

================
ダイモス登场机体
================

括号内为PS版变动。地形补正(→)为用默认驾驶员的地形适应和机体的移动类型修正之后的数据。偏差值表示排名位置，均值为50。

.. _srw4_units_daimos_commentBegin:


.. _srw4_units_daimos_commentEnd:

--------
我军机体
--------
.. _srw4_units_daimos_own_commentBegin:


.. _srw4_units_daimos_own_commentEnd:

^^^^^^^^
ダイモス
^^^^^^^^

.. _srw4_unit_daimos:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_35.png

        登场/加入:第7话

    .. grid-item::
        :columns: auto

        | HP 4000
        | 我军偏差值 53.21(53.2)
        | 整体偏差值 46.31(46.31)
        | EN 200
        | 我军偏差值 56.67(56.63)
        | 整体偏差值 52.48(52.47)
        | 装甲 380
        | 我军偏差值 50.3(49.79)
        | 整体偏差值 49.03(48.85)
        | 运动性 36
        | 我军偏差值 49.12(49.05)
        | 整体偏差值 50.7(50.54)
        | 限界 180
        | 我军偏差值 48.2(48.16)
        | 整体偏差值 48.42(48.38)

    .. grid-item::
        :columns: auto

        | 编码 35
        | Snes地址 B9EB3
        | 武器首地址 B9ED3
        | PS地址 26BA2
        | 类型 空陸
        | 移动力 7
        | 我军偏差值 42.48(42.48)
        | 整体偏差值 44.66(44.66)
        | 大小 L

        | 经验值 210
        | 我军偏差值 62.04(62.04)
        | 整体偏差值 61.77(61.77)
        | 价值 4000
        | 我军偏差值 50.66(50.66)
        | 整体偏差值 49.02(49.02)
        | 修理费 5000
        | 我军偏差值 49.35(49.35)
        | 整体偏差值 47.92(47.92)
    .. grid-item::
        :columns: auto

        | 空B (A)→B
        | 陆A
        | 海D (C)
        | 宇B

        .. image:: ../pilots/images/srw4_pilot_6B.png

        | 地形参照
        | :ref:`竜崎一矢 <srw4_pilots_kazuya_ryuzaki>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: スネークロック⚔Ⓟ
        :columns: 3

    .. grid-item:: 800

    .. grid-item:: 1~2

    .. grid-item:: +12

    .. grid-item:: 

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ミサイル⚔
        :columns: 3

    .. grid-item:: 840

    .. grid-item:: 1~5

    .. grid-item:: 

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 5

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ダイモガン
        :columns: 3

    .. grid-item:: 870(970)

    .. grid-item:: 1~4

    .. grid-item:: -5

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 5

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾌｧｲﾌﾞｼｭｰﾀｰ⚔Ⓟ
        :columns: 3

    .. grid-item:: 910(1010)

    .. grid-item:: 1~3

    .. grid-item:: +14

    .. grid-item:: +20

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: フットカッター🤛Ⓟ
        :columns: 3

    .. grid-item:: 950

    .. grid-item:: 1

    .. grid-item:: +10

    .. grid-item:: +20

    .. grid-item:: 空A→B陆A海A→C (B)宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: クロスブーメラン⚔Ⓟ
        :columns: 3

    .. grid-item:: 970(1070)

    .. grid-item:: 1~3

    .. grid-item:: +5

    .. grid-item:: +10

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 真空回し蹴り🤛Ⓟ
        :columns: 3

    .. grid-item:: 1100

    .. grid-item:: 1

    .. grid-item:: 

    .. grid-item:: +20

    .. grid-item:: 空A→B陆A海A→C (B)宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 必要气力 110

    .. grid-item::
        :class: flex-break

    .. grid-item:: 双竜剣🤛Ⓟ
        :columns: 3

    .. grid-item:: 1270(1370)

    .. grid-item:: 1

    .. grid-item:: +16

    .. grid-item:: +30

    .. grid-item:: 空A→B陆A海A→C (B)宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 必要气力 110

    .. grid-item::
        :class: flex-break

    .. grid-item:: ドリルアンカー⚔
        :columns: 3

    .. grid-item:: 1350(1450)

    .. grid-item:: 1~5

    .. grid-item:: -5

    .. grid-item:: +20

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 三竜棍🤛Ⓟ
        :columns: 3

    .. grid-item:: 1450(1550)

    .. grid-item:: 1

    .. grid-item:: +8

    .. grid-item:: +10

    .. grid-item:: 空A→B陆A海A→C (B)宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 必要气力 120

    .. grid-item::
        :class: flex-break

    .. grid-item:: ダイモシャフト🤛Ⓟ
        :columns: 3

    .. grid-item:: 1680

    .. grid-item:: 1

    .. grid-item:: +10

    .. grid-item:: +20

    .. grid-item:: 空A→B陆A海A→C (B)宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 必殺烈風正拳突きⓅ
        :columns: 3

    .. grid-item:: 2900(3200)

    .. grid-item:: 1

    .. grid-item:: +18 (+25)

    .. grid-item:: +30

    .. grid-item:: 空A陆A海A宇🚫
        :columns: 3

    .. grid-item:: EN 70

    .. grid-item:: 必要气力 130

    .. grid-item::
        :class: flex-break

    .. grid-item:: 必殺烈風正拳突き改🤛Ⓟ
        :columns: 3

    .. grid-item:: 3700(4000)

    .. grid-item:: 1

    .. grid-item:: +25

    .. grid-item:: +30

    .. grid-item:: 空A→B陆A海A→C (B)宇🚫
        :columns: 3

    .. grid-item:: EN 80

    .. grid-item:: 必要气力 135

    .. grid-item::
        :class: flex-break

.. _srw4_unit_daimos_commentBegin:

开局之后加入部队的第一个超级机器人。当然也是最弱的一个。装甲比较差，所以虽然可以飞，但是只有在陆地上才能发挥最大能力（默认移动方式是飞行），所以开战之后还是陆行吧。

本作近身格斗武器的地形适应是看敌方所在，所以用必殺烈風正拳突き改攻击空中敌人的时候攻防能力要下降两成，用盖塔的烈日光弹攻击地面敌人的时候也会吃这个亏。不过必殺烈風正拳突き居然是近身射击武器，没有这个惩罚，大概算个bug。

戴莫斯远程的武器攻击低残弹少。近身的武器虽然很多，但是重复性太高，除了大招之外，就连需要涨气力之后才能用的武器攻击力也才一千出头，基本都没有什么改造价值，除了暴击率+30的双竜剣。作为单座机器人，缺乏幸运和加速，大招也不能在宇宙中使用，后期出场价值较低。

走超级系路线的话，30话会追加几个实用武器，其中必殺烈風正拳突き改EN消耗不高比较实用。走真实系路线的话，因为没有追加武器，这机体并不实用。会在宇宙中出战两话，大招在宇宙里都不能用基本就让这台机器人废掉了。

第四次S里面武器做了强化，但是仍然不能在宇宙里用大招。地形适应升级成空A海C，但是一矢仍然是空B，造成对空能力无变化。

本作没有强制入海的地图，在有空B的场合，无论海C还是海D，海洋都是不会主动进入的地形。地形适应的设定更改可以说是基本没有实际意义……

.. _srw4_unit_daimos_commentEnd:

^^^^^^^^^^^^^^
ガルバーFXⅡ
^^^^^^^^^^^^^^

.. _srw4_unit_galva_fx_ii:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_36.png

        登场/加入:第7话

    .. grid-item::
        :columns: auto

        | HP 1300
        | 我军偏差值 42.12(42.11)
        | 整体偏差值 44.04(44.06)
        | EN 110
        | 我军偏差值 30.23(30.22)
        | 整体偏差值 28.72(28.7)
        | 装甲 100
        | 我军偏差值 35.32(35.91)
        | 整体偏差值 37.06(37.56)
        | 运动性 41
        | 我军偏差值 54.75(54.7)
        | 整体偏差值 55.83(55.61)
        | 限界 175
        | 我军偏差值 46.66(46.61)
        | 整体偏差值 46.9(46.88)

    .. grid-item::
        :columns: auto

        | 编码 36
        | Snes地址 B9EFF
        | 武器首地址 B9F1F
        | PS地址 26BEE
        | 类型 空
        | 移动力 8
        | 我军偏差值 47.89(47.89)
        | 整体偏差值 50.04(50.04)
        | 大小 S

        | 经验值 80
        | 我军偏差值 40.78(40.78)
        | 整体偏差值 41.84(41.84)
        | 价值 1000
        | 我军偏差值 45.02(45.02)
        | 整体偏差值 43.76(43.76)
        | 修理费 800
        | 我军偏差值 44.66(44.66)
        | 整体偏差值 44.38(44.38)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆🚫→D
        | 海🚫→D
        | 宇B

        .. image:: ../pilots/images/srw4_pilot_4F.png

        | 地形参照
        | :ref:`桜野マリ <srw4_pilots_mari_sakurano>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: 補給装置🔄
        :columns: 3

    .. grid-item:: 0

    .. grid-item:: 1~3

    .. grid-item:: 

    .. grid-item:: 

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 20ミリバルカンⓅ
        :columns: 3

    .. grid-item:: 300

    .. grid-item:: 1

    .. grid-item:: +30

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ミサイル⚔
        :columns: 3

    .. grid-item:: 830

    .. grid-item:: 1~5

    .. grid-item:: 

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 4

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_galva_fx_ii_commentBegin:

基本没有战斗能力，敌方集火的目标，因为战斗机机师们也需要练级，所以还是要改造一下。本作中在换乘时属于戦闘機系，没有分队的时候，勇者ライディーン系的战斗机驾驶员们可以换乘这部机体出来练级（戦闘機系另一部补给机的能力要差一点）。

.. _srw4_unit_galva_fx_ii_commentEnd:

--------
敌军机体
--------
.. _srw4_units_daimos_enemy_commentBegin:


.. _srw4_units_daimos_enemy_commentEnd:

^^^^^^^^^^^^^^^^^^^^^^
メカ戦士ゾンネカイザー
^^^^^^^^^^^^^^^^^^^^^^

.. _srw4_unit_mecha_warrior_sonnekaiser:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_C3.png

        登场/加入:第7话

    .. grid-item::
        :columns: auto

        | HP 5800
        | 我军偏差值 60.6(60.59)
        | 整体偏差值 47.82(47.8)
        | EN 200
        | 我军偏差值 56.67(56.63)
        | 整体偏差值 52.48(52.47)
        | 装甲 280
        | 我军偏差值 44.95(44.83)
        | 整体偏差值 44.75(44.82)
        | 运动性 27
        | 我军偏差值 38.99(38.89)
        | 整体偏差值 41.46(41.4)
        | 限界 160
        | 我军偏差值 42.01(41.97)
        | 整体偏差值 42.37(42.35)

    .. grid-item::
        :columns: auto

        | 编码 C3
        | Snes地址 BB74A
        | 武器首地址 BB76A
        | PS地址 28439
        | 类型 空陸
        | 移动力 7
        | 我军偏差值 42.48(42.48)
        | 整体偏差值 44.66(44.66)
        | 大小 L

        | 经验值 80
        | 我军偏差值 40.78(40.78)
        | 整体偏差值 41.84(41.84)
        | 价值 2400
        | 我军偏差值 47.65(47.65)
        | 整体偏差值 46.21(46.21)
        | 修理费 5000
        | 我军偏差值 49.35(49.35)
        | 整体偏差值 47.92(47.92)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆A
        | 海A
        | 宇B

        .. image:: ../pilots/images/srw4_pilot_AE.png

        | 地形参照
        | :ref:`バルバス将軍 <srw4_pilots_balbas>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ランスミサイル⚔
        :columns: 3

    .. grid-item:: 920

    .. grid-item:: 1~6

    .. grid-item:: -5

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 4

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ビームランス🤛Ⓟ
        :columns: 3

    .. grid-item:: 1280

    .. grid-item:: 1

    .. grid-item:: +5

    .. grid-item:: +10

    .. grid-item:: 空A陆A海C宇A→B
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 大型ミサイル⚔
        :columns: 3

    .. grid-item:: 1620

    .. grid-item:: 1~6

    .. grid-item:: -22

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 2

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mecha_warrior_sonnekaiser_commentBegin:

攻强守弱的突击队员。还是早点干掉比较好。一开始是バルバス将軍的座机，后期就成为了バーム軍的量产杂鱼机。

.. _srw4_unit_mecha_warrior_sonnekaiser_commentEnd:

^^^^^^^^^^^^
ガルンロール
^^^^^^^^^^^^

.. _srw4_unit_guranrol:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_11C.png

        登场/加入:第7话

    .. grid-item::
        :columns: auto

        | HP 23000
        | 我军偏差值 131.25(131.21)
        | 整体偏差值 62.28(62.09)
        | EN 255
        | 我军偏差值 72.83(72.77)
        | 整体偏差值 67(66.99)
        | 装甲 430
        | 我军偏差值 52.98(52.27)
        | 整体偏差值 51.17(50.87)
        | 运动性 22
        | 我军偏差值 33.36(33.25)
        | 整体偏差值 36.33(36.32)
        | 限界 200
        | 我军偏差值 54.39(54.35)
        | 整体偏差值 54.46(54.41)

    .. grid-item::
        :columns: auto

        | 编码 11C
        | Snes地址 BC68C
        | 武器首地址 BC6AC
        | PS地址 29378
        | 类型 空陸
        | 移动力 7
        | 我军偏差值 42.48(42.48)
        | 整体偏差值 44.66(44.66)
        | 大小 LL

        | 经验值 215
        | 我军偏差值 62.86(62.86)
        | 整体偏差值 62.54(62.54)
        | 价值 14000
        | 我军偏差值 69.44(69.44)
        | 整体偏差值 66.56(66.56)
        | 修理费 14000
        | 我军偏差值 59.4(59.4)
        | 整体偏差值 55.5(55.5)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆🚫→D
        | 海🚫→C
        | 宇A

        .. image:: ../pilots/images/srw4_pilot_AD.png

        | 地形参照
        | :ref:`リヒテル提督 <srw4_pilots_richter>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ショックビームⓅ
        :columns: 3

    .. grid-item:: 1000

    .. grid-item:: 1

    .. grid-item:: -5

    .. grid-item:: -10

    .. grid-item:: 空A陆A海C宇A
        :columns: 3

    .. grid-item:: EN 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ミサイル⚔
        :columns: 3

    .. grid-item:: 1070

    .. grid-item:: 1~6

    .. grid-item:: -15

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 20

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 破壊光線⚔
        :columns: 3

    .. grid-item:: 1180

    .. grid-item:: 1~6

    .. grid-item:: -5

    .. grid-item:: 

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 20

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_guranrol_commentBegin:

敌军里比较弱的战舰，大招对海能力略差。

.. _srw4_unit_guranrol_commentEnd:

^^^^^^^^^^^^^^^^
メカ戦士ギメリア
^^^^^^^^^^^^^^^^

.. _srw4_unit_mecha_warrior_gimeria:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_C4.png

        登场/加入:第29话

    .. grid-item::
        :columns: auto

        | HP 6000
        | 我军偏差值 61.43(61.41)
        | 整体偏差值 47.99(47.97)
        | EN 210
        | 我军偏差值 59.61(59.57)
        | 整体偏差值 55.12(55.11)
        | 装甲 780
        | 我军偏差值 71.71(69.62)
        | 整体偏差值 66.13(64.98)
        | 运动性 32
        | 我军偏差值 44.62(44.54)
        | 整体偏差值 46.59(46.47)
        | 限界 190
        | 我军偏差值 51.3(51.25)
        | 整体偏差值 51.44(51.4)

    .. grid-item::
        :columns: auto

        | 编码 C4
        | Snes地址 BB775
        | 武器首地址 BB795
        | PS地址 28464
        | 类型 空陸
        | 移动力 7
        | 我军偏差值 42.48(42.48)
        | 整体偏差值 44.66(44.66)
        | 大小 L

        | 经验值 140
        | 我军偏差值 50.59(50.59)
        | 整体偏差值 51.04(51.04)
        | 价值 8000
        | 我军偏差值 58.17(58.17)
        | 整体偏差值 56.03(56.03)
        | 修理费 5000
        | 我军偏差值 49.35(49.35)
        | 整体偏差值 47.92(47.92)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆A
        | 海A
        | 宇A
        | 激怒/自爆/てかげん無効

        .. image:: ../pilots/images/srw4_pilot_B0.png

        | 地形参照
        | :ref:`アイザム <srw4_pilots_aizam>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ランスミサイル⚔
        :columns: 3

    .. grid-item:: 920

    .. grid-item:: 1~6

    .. grid-item:: -5

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 4

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ムチ🤛Ⓟ
        :columns: 3

    .. grid-item:: 1220

    .. grid-item:: 1

    .. grid-item:: 

    .. grid-item:: +10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 破壊光線
        :columns: 3

    .. grid-item:: 2100

    .. grid-item:: 1~6

    .. grid-item:: -20

    .. grid-item:: 

    .. grid-item:: 空A陆A海B宇A
        :columns: 3

    .. grid-item:: 残弹 18

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 反ｱｲｻﾞﾛﾝ粒子砲Ⓟ
        :columns: 3

    .. grid-item:: 2970

    .. grid-item:: 1

    .. grid-item:: -8

    .. grid-item:: +20

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 10

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mecha_warrior_gimeria_commentBegin:


.. _srw4_unit_mecha_warrior_gimeria_commentEnd:

^^^^^^^^^^^^^^^^^^^^^^
メカ戦士ゴッドアーモン
^^^^^^^^^^^^^^^^^^^^^^

.. _srw4_unit_mecha_warrior_goddoamon:



.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item::
        :columns: auto

        .. image:: ../units/images/portrait/srw4_units_portrait_C5.png

        登场/加入:第43话

    .. grid-item::
        :columns: auto

        | HP 17000
        | 我军偏差值 106.61(106.58)
        | 整体偏差值 57.24(57.1)
        | EN 200
        | 我军偏差值 56.67(56.63)
        | 整体偏差值 52.48(52.47)
        | 装甲 420
        | 我军偏差值 52.44(51.78)
        | 整体偏差值 50.74(50.46)
        | 运动性 29
        | 我军偏差值 41.24(41.15)
        | 整体偏差值 43.51(43.43)
        | 限界 170
        | 我军偏差值 45.11(45.06)
        | 整体偏差值 45.39(45.37)

    .. grid-item::
        :columns: auto

        | 编码 C5
        | Snes地址 BB7A4
        | 武器首地址 BB7C4
        | PS地址 28493
        | 类型 空陸
        | 移动力 7
        | 我军偏差值 42.48(42.48)
        | 整体偏差值 44.66(44.66)
        | 大小 L

        | 经验值 200
        | 我军偏差值 60.41(60.41)
        | 整体偏差值 60.24(60.24)
        | 价值 6800
        | 我军偏差值 55.92(55.92)
        | 整体偏差值 53.93(53.93)
        | 修理费 10000
        | 我军偏差值 54.93(54.93)
        | 整体偏差值 52.13(52.13)
    .. grid-item::
        :columns: auto

        | 空A
        | 陆A
        | 海B
        | 宇A

        .. image:: ../pilots/images/srw4_pilot_AE.png

        | 地形参照
        | :ref:`バルバス将軍 <srw4_pilots_balbas>`


.. grid:: 
    :gutter: 3
    :margin: 0
    :padding: 0
    :outline:

    .. grid-item:: 名字
        :columns: 3

    .. grid-item:: 攻击

    .. grid-item:: 射程

    .. grid-item:: 命中

    .. grid-item:: 暴击

    .. grid-item:: 地形空陆海宇
        :columns: 3

    .. grid-item:: 残弹/EN

    .. grid-item:: 条件

    .. grid-item::
        :class: flex-break

    .. grid-item:: ミサイル⚔
        :columns: 3

    .. grid-item:: 1070

    .. grid-item:: 1~6

    .. grid-item:: -15

    .. grid-item:: -10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 20

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: ﾀﾞｰｸｻｰﾍﾞﾙ🤛Ⓟ
        :columns: 3

    .. grid-item:: 1200(1400)

    .. grid-item:: 1

    .. grid-item:: +10

    .. grid-item:: +20

    .. grid-item:: 空A陆A海A→B宇A
        :columns: 3

    .. grid-item:: 

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

    .. grid-item:: 超破壊光線
        :columns: 3

    .. grid-item:: 2200

    .. grid-item:: 1~8

    .. grid-item:: -10

    .. grid-item:: +10

    .. grid-item:: 空A陆A海A宇A
        :columns: 3

    .. grid-item:: 残弹 16

    .. grid-item:: 
    .. grid-item::
        :class: flex-break

.. _srw4_unit_mecha_warrior_goddoamon_commentBegin:


.. _srw4_unit_mecha_warrior_goddoamon_commentEnd:

