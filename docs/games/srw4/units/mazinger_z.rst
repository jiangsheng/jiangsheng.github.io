.. meta::
   :description: 括号内为PS版变动。地形补正(→)为用默认驾驶员的地形适应和机体的移动类型修正之后的数据。 魔神Z优点是武器消耗低，甚至连导弹都有20发，而且不需要提升气力就可以使用。但是大招攻击力太低，跟真实系的浮游炮是一个等级，而且射程是1，不要指望对BOSS有用……反正他也没幸运，是吧。 早期因为甲児有铁壁还是能撑一阵的，特别是
