.. meta::
   :description: 作者的话：GDI+看起来是更好的解决方案，但是IPicture的OLE实现更简单。 很久以来，我都被一个问题困扰。关于程序中显示图像的问题，我在网络上搜索了很长时间，找到了无数的解决方案，比如分析文件格式,直接读取文件的;用控件的(柯达的ImgEdit控件);以及不知道内部实现方法的库（ImageLoad）。而我找到的
