.. meta::
   :description: 问： 我在服务器上用 CreateFileMapping 创建了一段共享内存。让这个exe始终在服务器上跑。同时，别的用户在客户端用IE访问服务器，将要查询的数据通过C#制作的网页提交上来，服务器得到网页参数后，建立一个COM对象访问上一个exe的共享内存，然后将在共享内存中的查询结果返回给客户。问题是现在这个COM无
